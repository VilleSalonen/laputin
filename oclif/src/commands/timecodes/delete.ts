...
export default class DeleteTimecodesCommand extends Command {
    static description = 'Deletes timecodes';
    ...
    public async run(): Promise<void> {
        ...
        const { args, flags } = await this.parse(DeleteTimecodesCommand);
        ...
        const confirmation = await CliUx.ux.prompt('Delete timecodes (only yes will be accepted)');
        if (confirmation === 'yes') {
            await library.deleteTimecodes(timecodes);
        }
    }
